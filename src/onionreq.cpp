...
void pybind(pybind11::module m) {
    ...
    py::class_<PyOnionReqParser>(
            ...
            .def(py::init([](py::bytes pubkey_b,
                             ...
                             py::bytes request_b,
                             size_t max_size) {
                     auto pubkey = bytes_from_pybytes(pubkey_b, "x25519_pubkey", 32);
                     auto privkey = bytes_from_pybytes(privkey_b, "x25519_privkey", 32);
                     auto request = bytes_from_pybytes(request_b);
                     PyOnionReqParser parser{pubkey, privkey, request, max_size};
                     auto pl = parser.move_payload();
                     parser.payload_b =
            ...
            .def(
                    ...
                    [](const PyOnionReqParser& parser, py::bytes reply) {
                        auto encr = parser.encrypt_reply(bytes_from_pybytes(reply));
